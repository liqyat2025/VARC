...
for (( gpu=0; gpu<NUM_GPUS; gpu++ )); do
  (
    ...
    for file_name in "${file_names[@]}"; do
      # Assign roughly 1/8 of tasks to each GPU by index
      if (( i % NUM_GPUS == gpu )); then
        echo "GPU ${gpu} processing task ${file_name} (ARC-1 VARC-Unet)"
        CUDA_VISIBLE_DEVICES=${gpu} python test_time_train_ARC.py \
          ...
          --architecture "unet" \
          --eval-save-name "ARC_1_eval_Unet" \
          --num-attempts 10 \
          --ttt-num-each 2
      fi
      ((i++))
...
wait
echo "All VARC-Unet ARC-1 finished."